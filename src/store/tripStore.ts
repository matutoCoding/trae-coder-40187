...
interface TripStore {
  ...
  addChecklistItem: (group: ChecklistItem['group'], category: string, name: string, isDaily?: boolean) => void
...
export const useTripStore = create<TripStore>((set, get) => ({
  ...
  addChecklistItem: (group, category, name, isDaily = false) => {
    set((state) => ({
      checklist: [
        ...state.checklist,
        {
  ...
  exportChecklist: () => {
    ...
    const buildGroupedText = (items: typeof checklist, sectionLabel?: string) => {
      let text = ''
      if (sectionLabel) {
        text += `\n━━━━━━━━━━━━━━━━━━━━━━━━\n${sectionLabel}\n━━━━━━━━━━━━━━━━━━━━━━━━\n`
      }
      ...
      Object.entries(groupedByGroup).sort((a, b) => {
        ...
        text += `\n【${data.groupLabel}】\n`
        Object.entries(data.categories).forEach(([cat, catItems]) => {
    ...
    if (!itinerary) {
      let text = '🚗 路书行 - 出发清单\n\n'
      const dailyItems = checklist.filter((i) => i.isDaily)
      const generalItems = checklist.filter((i) => !i.isDaily)
      if (dailyItems.length > 0) {
        text += buildGroupedText(dailyItems, '📅 按天准备的物品')
      }
      if (generalItems.length > 0) {
        text += buildGroupedText(generalItems, '🎒 通用物品')
      }
      return text
    }
...
💰 预估费用：${itinerary.costEstimate?.totalCostRange || '—'}
━━━━━━━━━━━━━━━━━━━━━━━━
...
`
    ...
    let text = header
    if (dailyItems.length > 0) {
      text += buildGroupedText(dailyItems, '📅 按天准备（每天都要检查/携带）')
    }
    if (generalItems.length > 0) {
      text += buildGroupedText(generalItems, '🎒 通用物品（出行前一次备齐）')
    }
